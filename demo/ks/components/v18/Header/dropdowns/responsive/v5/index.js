...
class KSDropdownResponsive extends HTMLElement {
    connectedCallback() {
        ...
        const button = createButton(this, defaults);
        const { btnContent, chevronDiv } = createBtnContent(this, defaults);
        button.appendChild(btnContent);

        const menu = createMenu(defaults);
        populateMenu(menu, Array.from(this.children), defaults);
...
window.ks.components.dropdown.responsive.version = "v4";
